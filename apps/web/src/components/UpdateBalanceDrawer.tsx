...
export default function UpdateBalanceDrawer({
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      gooeyToast.success("Số dư đã cập nhật", {
        description: new Intl.NumberFormat("vi-VN", { style: "currency", currency: "VND", maximumFractionDigits: 0 }).format(resultBalance),
      });
      ...
      gooeyToast.error("Lỗi kết nối", { description: "Vui lòng thử lại." });
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <div className="fixed bottom-20 left-0 right-0 z-50 glass-panel-elevated rounded-t-3xl p-6 pb-10 animate-in slide-in-from-bottom duration-300">
        {/* Handle */}
        <div className="w-10 h-1 bg-[rgba(125,211,252,0.2)] rounded-full mx-auto mb-6" />

        <div className="flex justify-between items-start mb-6">
          <div>
            <h2 className="text-2xl font-bold text-[#e0e8f0]">Điều chỉnh số dư</h2>
        ...
        <div className="mb-4 text-center">
          <p className="text-[10px] font-bold uppercase tracking-widest text-[#a0b4c4] mb-1">Số dư sau điều chỉnh</p>
          <span className={`text-3xl font-extrabold ${resultBalance < 0 ? "text-red-400" : "text-[#7dd3fc]"}`}>
            {new Intl.NumberFormat("vi-VN", { style: "currency", currency: "VND", maximumFractionDigits: 0 }).format(resultBalance)}
          </span>
        </div>
