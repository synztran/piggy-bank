		...
		<PullToRefresh onRefresh={fetchData}>
			<div className="space-y-6 pt-4">
              ...
              +2.4%
						...
						{/* Member balances */}
						{!loading &&
							(summary?.memberBalances?.length ?? 0) > 1 && (
					...
					<div className="grid grid-cols-2 gap-4">
						{topSpending.length === 0 && !loading && (
							<div className="col-span-2 glass-panel p-5 rounded-xl text-center text-[#a0b4c4] text-sm">
								Chưa có dữ liệu chi tiêu. Hãy thêm giao dịch đầu
								tiên!
							</div>
						)}
						{topSpending.map((item) => {
							const Icon =
								categoryIcon[item.category] || ShoppingBag;
							const textColor =
								categoryColor[item.category] ||
								"text-slate-400";
							const barColor =
								categoryBarColor[item.category] ||
								"bg-slate-400";
							const bgClass =
					...
					<div className="space-y-3">
						...
						{!loading &&
							summary?.recentTransactions.map((tx) => {
								const Icon =
									categoryIcon[tx.category] || ShoppingBag;
								const bgClass =
